...
class PropiedadesModel extends Model{
    ...
    public $identificador   = "propiedad_id";
    public $creador         = "propiedad_creadopor";
    public $actualizador    = "propiedad_actualizadopor";
    public $factualizacion  = "propiedad_factualizacion";
    public $sqlEstado       = "propiedad_estado";
    ...
    public function get($user,$id = null){
        if($id == null){
            $sql = "SELECT 
                        pr.*,
                        co.complejo_nombre,
                        coalesce(pe.persona_nombre1 ||' '|| pe.persona_apellido1 || ' '|| pe.persona_apellido2,pe.persona_nombre1 ||' '|| pe.persona_apellido1) as persona_nombre,
                        ed.edificio_nombre
                    from 
                        propiedades pr
                        ...
                        left join edificios ed
                            on ed.edificio_id = pr.edificio_id
                        left join ( select 
								   		us.usuario_id,
								  		pe.persona_nombre1,
								   		pe.persona_apellido1,
								   		pe.persona_apellido2
								 	from usuarios us 
							...
							on pe.usuario_id = pr.usuario_id
                    WHERE
                    ...
                    SELECT 
                        pr.*,
                        co.complejo_nombre,
                        coalesce(pe.persona_nombre1 ||' '|| pe.persona_apellido1,pe.persona_nombre1) as persona_nombre,
                        ed.edificio_nombre
                    from 
                        propiedades pr
                        inner join complejos co
    ...
    public function getParamsUpdate(){
        $sql = "SELECT 
                    ...
                    from (select 
                            ...
                            coalesce(pe.persona_nombre1 || ' ' ||pe.persona_nombre2 ||' '||pe.persona_apellido1|| ' '|| pe.persona_apellido2,
                            pe.persona_nombre1 || ' ' ||pe.persona_apellido1 || ' ' || pe.persona_apellido2,
                            pe.persona_nombre1 || ' ' ||pe.persona_nombre2 || ' ' ||pe.persona_apellido1,
                            pe.persona_nombre1 || ' ' ||pe.persona_apellido1) ||' - '|| us.usuario_username as nombre
                        from
                            usuarios us
